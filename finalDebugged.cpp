...
void initializePheromone(solution S0,Server b[],ll m, VM a[], ll n);
...
void prepare(qualifiedVM& t,int nextVM,solution antSol,ll n,int nextServer,ll m);  //will use 1,2 to update t
double drawQ();
int exploitation(map<ll,qualifiedVM> omega);   //return maximum of probability
ll exploration(map<ll,qualifiedVM> omega);    //return random number according to probability
...
void globalPheromoneUpdate(solution s,ll t,ll n);
...
int main() {
    ...
    if(!check(n,m))                    //Checking whether input data is in limit or not
    {
		...
		for(register int k=1;k<=NO_OF_ANTS;k++)
		{
			...
			for(int j=0;j<m;j++)
			{
				...
				while(true)
				{
					...
					for(register int i=0;i<n;i++)
					{
						if(compatibleVM(antSol,i,n,serverClone[j].serverID,serverClone[j],omega))
						{
...
void prepare(qualifiedVM& t,int nextVM,solution antSol,ll n,int nextServer,ll m)
{
	double sum[9][MAX];
